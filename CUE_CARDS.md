...
| **6** Design (+ window) | 1:30 | "The fix — it's about *where* a re-hit happens." | Add a **correlation window**: in-window hits don't set Ref → Ghost; beyond → Ref=1 → Main. One rule on top of S3-FIFO. |
| **7** Animation | 1:15 | "Watch the one change vs. S3-FIFO." *(Next ▶ / click)* | Window keeps the burst's Ref=0 → Ghost; S3-FIFO would promote it (pollution). |
